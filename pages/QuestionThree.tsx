...
export default function QuestionThree({navigation}: {navigation: any}) {
  ...
  const save = async () => {
    ...
    try {
      const value = await AsyncStorage.getItem('agr');

      if (value !== null) {
        await AsyncStorage.removeItem('agr');
        const jsonValue = JSON.stringify(data.values);
        await AsyncStorage.setItem('agr', jsonValue);
  ...
  return (
    <>
      <SafeAreaView style={styles.container}>
        <StatusBar backgroundColor="#E6E6FA" barStyle="light-content" />
        <View style={{marginLeft: 10, marginRight: 10}}>
          ...
          <View style={{marginTop: 30}}>
            <ProgressBar progress={0.50} color="#8E44AD" style={{height: 7, backgroundColor: "white"}} />
          </View>
